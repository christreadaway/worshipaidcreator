...
Bridging (an even-page music image continuing onto the facing odd page as a lossless clipped two-part drawing) shipped in v1.7 and was **retired in v1.7.1** after June 2026 parish feedback: in practice the sections that follow the image in render order printed *between* the two halves (processional split around the Penitential Act; communion hymn split around the Choral Anthem), breaking blocks the assembly reads as one piece. The mechanism (`_notationImage`'s `bridge` option, `_carryNotation`, `_drawCarriedNotation`) remains in the code but **no caller enables it**. Current hard rules: **music blocks stay whole on one page** — a too-tall image shrinks proportionally (complete, never cut) — and **nothing is ever dropped silently**: every content page runs through `_fitPageText` shrink-to-fit, and anything that still cannot fit is truncated *with a warning that is surfaced to the user at export* (JSON `warnings` locally; `X-Export-Warnings` response header on Netlify; both rendered as banners in the editor).
