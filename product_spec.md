...
> **v2.4 — one shared liturgy outline (the comprehensive de-duplication) + review pass.** The liturgy structure used to exist in **four copies** (reimagined/classic × PDF/HTML), which is why the same proof feedback kept recurring: a rule fixed in one copy drifted in the others (preview-vs-print, design-vs-design). New `src/liturgy-outline.js` — `buildLiturgyOutline(data, ctx)` returns an ordered list of typed liturgy "ops" and is the **single source of truth** for section order, section names, posture wording, which items appear, two-column choices, announcements placement, the QR footer, and every reimagined-vs-classic difference. The PDF generator (`_blocksFromOutline` + `_renderOp`) and the HTML preview (`renderOpHtml`) both consume it; `_buildContentBlocksClassic` is now a one-line delegate. ~400 lines of duplicated structure deleted, PDF output byte-identical (verified page-by-page, all seasons/designs). A formatting rule is now written **once** and applies everywhere. Hardened by a four-angle adversarial review (outline parity via a 52-combo diff, PDF-flow 192-config fuzz, HTML/XSS, server/strip pipeline): fixed a MEDIUM title-crop regression that could remove a narrow real staff (music loss), plus LOW fixes — the local `?strip=1` route now guards to png/jpeg, an absurdly long feast name can no longer force a 9th page (cover-title height clamps), `_twoColumnText` warns on truncation, and the classic page-8 primitives gained past-bottom guards. 423 unit + 5 E2E passing. The director's 17th-Sunday proof items (5″ notation, roman composer names, ≥8pt heading spacing, Penitential Act on the entrance-hymn page, Kyrie+Gloria+Collect together, render-time title stripping) are preserved in **both** designs.

> **v2.3 — director-of-liturgy proof pass #3 (17th Sunday in Ordinary Time).** Applied to both renderers: the default music-notation width is **5″** (was 5.5″); a music line's **composer name is never italicized** (title stays italic) via typed italic/roman runs; **≥8pt of space above every heading**; the **Penitential Act shares the entrance-hymn page** (a fit block: full text → two-column → heading-only-with-warning, never spilling); **Lord Have Mercy + Gloria + Collect render together on one page** (one atomic keep-with-next block); and notation **title headers are stripped at render time**, not only at upload, so scans stored before the cropper existed print clean. New `src/tests/proof-fixes3.test.js`.
...
> **Pick-up note for next session:** v2.2 is a **comprehensive review + hardening pass** over the v2.0/v2.1 work: an 8-angle adversarial code review (10 verified findings, all fixed) plus a layout stress-harness (both designs × 6 content scenarios × 2 trims with orphan/strand detection). Highlights — **correctness:** the editor UI was silently pinning rubric alignment to `'left'` at three layers, so the director's centered-rubric fix never reached real exports (now center everywhere, with a string-guard test); classic preview fonts 404'd on Netlify (the build now publishes them under `public/assets/fonts/classic/`); classic export crashed on an empty feast name; a phantom "Communion Antiphon" heading + empty paste box printed on every classic export that the preview never showed (removed); classic now handles Lent ("Gospel Acclamation" heading + sung-text fallback), the Advent wreath, cover `welcomeMessage`, children's liturgy, and preview overflow banners; the exported `design` is stamped on the draft + export-log and the pastor-approval gate pins it. **Layout:** new **keep-with-next pagination** (`keepNext` block flag honored identically by the measure and render passes — headings/transitions can never strand at a page foot; replaces the unsplittable Collect mega-block), and the classic QR/licensing footer **anchors to the foot of page 8** like a back cover. **Structure:** new `src/render-shared.js` gives both renderers one source of truth for QR loading, small-caps connectors, parish short-name, classic cover fallback copy, and children's-liturgy times; classic posture wording lives beside RUBRICS in `mass-texts.js` (`RUBRICS_CLASSIC`); `renderBookletHtml` builds only the selected body (classic CSS/QR work no longer runs on reimagined previews). 399 tests + 5 E2E passing. **Known debt:** the liturgy *structure* still exists in 4 copies (reimagined/classic × PDF/HTML) — a shared "liturgy outline" abstraction is the right next refactor.
>
> **Prior:** v2.1 adds **two selectable output designs**. The editor's left panel has a **Design** switcher: **"Reimagined"** (the app's original navy/gold/serif look — unchanged and still the default) and **"Classic Design"** (a faithful, freely-licensed emulation of the parish's in-house Book Antiqua / Garamond, monochrome worship aid). One flow engine, two themes: `pdf-generator.js` gained a `THEMES` layer (reimagined reproduces the original exactly; classic swaps fonts/colors) plus classic-only `renderPage1CoverClassic()`, `_buildContentBlocksClassic()`, small-caps headers, two-column psalm/creed, "Verse:" label, and a GIVE/JOIN/BULLETIN QR footer (`qrcode` dep, vector-drawn). Classic fonts are vendored under `src/assets/fonts/classic/` (TeX Gyre Pagella, EB Garamond, URW Bookman — OFL/GUST). The `design` field threads through `/api/preview`, `/api/generate-pdf`, `buildData()`, and per-user prefs. The **HTML preview and the PDF export both render the classic design faithfully** — `renderBookletHtml` splits the shared `<head>` from a per-design body (reimagined markup unchanged; classic gets its own body with the vendored fonts served via `/assets/fonts/classic/`, small-caps headers, two-column psalm/creed, "Verse:" line, and an inline-SVG QR footer) — differing only in the fixed-page-vs-flow pagination the HTML preview has always used. 388 tests passing (new `classic-design.test.js`).
>
> **Prior:** v2.0 is the **second director-of-liturgy proof pass** (13th Sunday in Ordinary Time re-proof). On `claude/liturgy-director-latest-v7mmyt`. All six code items from the re-proof were applied to BOTH renderers (`pdf-generator.js` + `template-renderer.js`) and verified by regenerating from live USCCB readings: (1) standalone posture directions are now **centered** (`rubric()`/`rubricAlign` default `left → center`; the right-justified-on-heading postures are unchanged); (2) the **Collect** heading no longer strands at a page foot — it renders in one block with "Please be seated" + *The Liturgy of the Word*; (3) **psalm strophes** are preserved when USCCB divides them with the refrain line rather than a blank line (fix in `splitPsalm`), so each strophe prints as its own "R."-capped block; (4) added a **Prayer after Communion** heading after the Choral Anthem with a centered "Please stand" above it; (5) the redundant "Please stand" before *Blessing & Dismissal* was removed (moved earlier); (6) **"Please kneel" now stays with the Lamb of God** so it can't orphan. Suite green (**373 tests**; new "Proof fixes v2" block + `splitPsalm` strophe test). **Still open (not a code bug):** the notation title-crop is an **upload-time** step — scans still showing a title / crop residue (proof pages 2, 5, 8) must be **re-uploaded** (with "Remove title headers" checked) to re-run the crop; regenerating the aid does not re-process stored images. The repo has no real notation fixtures, so `TITLE_CROP` was left untuned — share the failing scans to tune it safely.
>
...
> - **Notation title-crop** now runs at **render time** as well as at upload
>   (v2.3): every notation image is re-cropped when the booklet is generated
>   (idempotent, content-hash cached), so scans stored before the cropper
>   existed/improved print clean without needing a re-upload. The preview
>   requests the cropped variant via `?strip=1`. The over-crop guard (v2.4)
>   refuses a tall crop when a narrow real staff sits above the cut, so music
>   is never lost.
